...
pub export fn syscallHandler(context: *interrupts.CpuState) void {
    ...
    log.debug("syscall:  eax(syscall number): 0x{X:0>8},  ebx(arg0): 0x{X:0>8}\n", .{
        context.eax,
        context.ebx,
    });
    log.debug("syscall:  ecx(arg1):           0x{X:0>8},  edx(arg2): 0x{X:0>8}\n", .{
        context.ecx,
        context.edx,
    });
    log.debug("syscall:  esi(arg3):           0x{X:0>8},  edi(arg4): 0x{X:0>8}\n", .{
        context.esi,
        context.edi,
    });
    log.debug("syscall:  ebp(arg5):           0x{X:0>8},  syscall: {s}\n", .{
        context.ebp,
        if (context.eax <= 385 and context.eax >= 0) syscallUtil.syscallNames[context.eax] else "unknown",
    });
    log.debug("syscall:  eax(syscall number): 0x{X:0>8},  syscall: {s}\n", .{
        context.eax,
        if (context.eax <= 385 and context.eax >= 0) syscallUtil.syscallNames[context.eax] else "unknown",
    });
    ...
    if (context.eax == 0xF3) {
        ...
        log.err("syscall:  not implumented syscall {s} number: 0x{X} \n", .{
            if (context.eax <= 385 and context.eax >= 0) syscallUtil.syscallNames[context.eax] else "unknown",
            context.eax,
        });
...
fn __syscall_set_framebuffer(framebuffer: ?[*]tty.FrameBuffer.Pixel, size: u32) u32 {
    log.debug("++__syscall_set_framebuffer()\n", .{});
    defer {
        log.debug("--__syscall_set_framebuffer()\n", .{});
    }
    if (framebuffer) |lframe| {
        ...
        tty.framebuffer.flushWithFrame(frameSlice) catch |err| {
            log.err("__syscall_set_framebuffer:  failed to flush framebuffer: {}\n", .{err});
            return syscallUtil.ERROR_RETURN;
        };
        return syscallUtil.SUCCESS_RETURN;
    } else {
        log.err("__syscall_set_framebuffer:  framebuffer is null\n", .{});
        return syscallUtil.ERROR_RETURN;
    }
}
...
fn __syscall_set_thread_area(userDesc: ?*user_desc) u32 {
    log.debug("++__syscall_set_thread_area()\n", .{});
    defer {
        log.debug("--__syscall_set_thread_area()\n", .{});
    }

    if (userDesc) |description| {
        log.debug("user_desc.entry_number = 0x{X}\n", .{description.entry_number});
        log.debug("user_desc.base_addr    = 0x{X}\n", .{description.base_addr});
        log.debug("user_desc.limit        = 0x{X}\n", .{description.limit});
        log.debug("user_desc.bitfeild     = {b}\n", .{@as(u32, @bitCast(description.bitfeild))});

        if (@as(u32, @bitCast(description.bitfeild)) == user_desc.EMPTY_FLAGS) {
            log.debug("user_desc is empty\n", .{});
            ...
            if (description.entry_number <= gdt.THREAD_TLS_END and description.entry_number >= gdt.THREAD_TLS_START) {
                ...
                log.info("set_thread_area:  set empty gdt entry in entry: #{}\n", .{description.entry_number});
                return syscallUtil.SUCCESS_RETURN;
            } else {
                log.err("__syscall_set_thread_area:  user_desc entry number is out of range\n", .{});
                return syscallUtil.ERROR_RETURN;
            }
        } else if (description.entry_number <= gdt.THREAD_TLS_END and description.entry_number >= gdt.THREAD_TLS_START) {
            log.debug("user_desc entry number is in range\n", .{});
            ...
            log.info("set_thread_area:  set entry: #{}, gdt entry {}\n", .{
                gdt.getGdtEntry(description.entry_number),
                description.entry_number,
            });
            return syscallUtil.SUCCESS_RETURN;
        } else if (description.entry_number == 0xFFFFFFFF) {
            log.debug("user_desc entry number is 0xFFFFFFFF(-1) also means i chose\n", .{});
            ...
            for (gdt.THREAD_TLS_START..gdt.THREAD_TLS_END) |i| {
                if (!gdt.isEntryPresent(i)) {
                    ...
                    log.info("set_thread_area:  set chosen entry: #{}, gdt entry {}\n", .{
                        gdt.getGdtEntry(description.entry_number),
                        description.entry_number,
                    });
                    return syscallUtil.SUCCESS_RETURN;
        ...
        log.err("__syscall_set_thread_area:  user_desc is null\n", .{});
        return syscallUtil.ERROR_RETURN;
    }
}
...
fn __syscall_mmap(addr: u32, length: u32, prot: u32, flags: u32, fd: u32, pgoffset: u32) u32 {
    ...
    if (addr == 0) {
        ...
        const retaddr: u32 = @intFromPtr(retSlice.ptr);
        log.info("__syscall_mmap:  addr is not null, mapped at: 0x{X} length: 0x{X}\n", .{ retaddr, retSlice.len });
        return retaddr;
    }
}

fn __syscall_brk(addr: u32) u32 {
    log.debug("++__syscall_brk()\n", .{});
    defer {
        log.debug("--__syscall_brk()\n", .{});
    }

    if (addr == 0) {
        log.info("__syscall_brk:  addr is null corrent break: 0x{X}\n", .{userThread.threadData.threadBreak});
        return userThread.threadData.threadBreak;
    } else if (addr >= memory.KERNEL_ADDRESS_SPACE) {
        log.err("__syscall_brk:  it wanted to expand to kernel: 0x{X} nughty nughty\n", .{addr});
        return userThread.threadData.threadBreak;
        ...
        const retSlice = vmm.mapVirtualAddressRange(userThread.threadData.threadBreak, alignedAddr - userThread.threadData.threadBreak) orelse {
            log.err("__syscall_brk:  failed to allocate pages\n", .{});
            return userThread.threadData.threadBreak;
        };
        log.info("__syscall_brk:  expand from 0x{X} to: 0x{X}\n", .{
            userThread.threadData.threadBreak,
...
fn __syscall_truncate(path: ?[*]u8, length: u32) u32 {
    if (path) |lpath| {
        log.debug("truncate:  path: {s}\n", .{lpath[0..length]});
        return syscallUtil.SUCCESS_RETURN;
    } else {
        log.debug("__syscall_truncate:  path is null?\n", .{});
        return syscallUtil.SUCCESS_RETURN;
    }
}
...
fn __syscall_open(path: ?[*:0]u8, flags: u32, mode: u32) u32 {
    _ = mode;
    log.debug("++__syscall_open()\n", .{});
    defer {
        log.debug("--__syscall_open()\n", .{});
    }
    if (path) |lpath| {
        log.debug("open:  path: {s}\n", .{lpath});
        const pathSlice = std.mem.span(lpath);
        const file = files.open(pathSlice, flags) catch |err| {
            log.err("__syscall_open:  failed to open file: {}\n", .{err});
            return syscallUtil.ERROR_RETURN;
        };
        log.info("__syscall_open:  opened file: {s} fd: {}\n", .{ pathSlice, file.fd });
        return file.fd;
    } else {
        log.debug("__syscall_open:  path is null?\n", .{});
        return syscallUtil.ERROR_RETURN;
    }
}

fn __syscall_close(fd: u32) u32 {
    log.debug("++__syscall_close()\n", .{});
    defer {
        log.debug("--__syscall_close()\n", .{});
    }
    if (fd > files.fds.len) {
        log.err("__syscall_close:  fd is out of range\n", .{});
        return syscallUtil.ERROR_RETURN;
    }

    files.close(fd) catch |err| {
        log.err("__syscall_close:  failed to close file: {}\n", .{err});
        return syscallUtil.ERROR_RETURN;
    };
    log.info("__syscall_close:  closed file: {}\n", .{fd});
    return syscallUtil.SUCCESS_RETURN;
}
...
fn __syscall_llseek(fd: u32, offset_high: u32, offset_low: u32, result: ?*u64, whence: u32) u32 {
    log.debug("++__syscall_llseek()\n", .{});
    defer {
        log.debug("--__syscall_llseek()\n", .{});
    }
    if (fd > files.fds.len) {
        log.err("__syscall_llseek:  fd is out of range\n", .{});
        return syscallUtil.ERROR_RETURN;
    }

    const returnPointer: *u64 = result orelse {
        log.err("__syscall_llseek:  result is null\n", .{});
        return syscallUtil.ERROR_RETURN;
    };
    if (files.fds[fd]) |*file| {
        log.debug("llseek:  fd: {}, offset_high: 0x{X}, offset_low: 0x{X}, result: 0x{X}, whence: {}\n", .{
            file.fd,
            offset_high,
            offset_low,
        ...
        if (@abs(offset) > file.file.len) {
            log.err("__syscall_llseek:  offset is out of range\n", .{});
            return syscallUtil.ERROR_RETURN;
        }
        switch (@as(OffsetType, @enumFromInt(whence))) {
            OffsetType.SEEK_SET => {
                log.debug("llseek:  SEEK_SET\n", .{});
                if (offset < 0) {
                    log.err("__syscall_llseek:  offset is negative\n", .{});
                    return syscallUtil.ERROR_RETURN;
                }
                file.offset = @intCast(offset);
                log.debug("__syscall_llseek:  set new offset: 0x{X}\n", .{file.offset});
            },
            OffsetType.SEEK_CUR => {
                log.debug("llseek:  SEEK_CUR\n", .{});
                ...
                log.debug("__syscall_llseek:  add to cur {} new offset: 0x{X}\n", .{ offset, file.offset });
            },
            OffsetType.SEEK_END => {
                log.debug("llseek:  SEEK_END\n", .{});
                if (offset > 0) {
                    log.err("__syscall_llseek:  offset is positive\n", .{});
                    return syscallUtil.ERROR_RETURN;
                }
                file.offset = file.file.len - @abs(offset);
                log.debug("__syscall_llseek:  from the end {} new offset: 0x{X}\n", .{ offset, file.offset });
            },
            else => {
                log.err("__syscall_llseek:  invalid whence\n", .{});
                return syscallUtil.ERROR_RETURN;
            },
        }
        returnPointer.* = file.offset;
        log.debug("__syscall_llseek:  return pointer content: 0x{X}\n", .{returnPointer.*});
        return syscallUtil.SUCCESS_RETURN;
    } else {
        log.err("__syscall_llseek:  fd is not valid\n", .{});
        return syscallUtil.ERROR_RETURN;
    }
}
...
fn __syscall_readv(fd: u32, iov: ?[*]IoVec, iovcnt: i32) u32 {
    log.debug("++__syscall_readv()\n", .{});
    defer {
        log.debug("--__syscall_readv()\n", .{});
    }
    if (fd > files.fds.len) {
        log.err("__syscall_readv:  fd is out of range: {}\n", .{fd});
        return syscallUtil.ERROR_RETURN;
    }
    if (iovcnt <= 0) {
        log.err("__syscall_readv:  iovcnt is below or equal 0: {}\n", .{iovcnt});
        return syscallUtil.ERROR_RETURN;
    }
    const liovcnt: u32 = @intCast(iovcnt);
    const file = files.fds[fd] orelse {
        log.err("__syscall_readv:  fd is not valid: {}\n", .{fd});
        return syscallUtil.ERROR_RETURN;
    };
    const iovSlice: []IoVec = (iov orelse {
        log.err("__syscall_readv:  iov is null\n", .{});
        return syscallUtil.ERROR_RETURN;
    })[0..liovcnt];
    var bytesRead: u32 = 0;
    for (iovSlice) |iovec| {
        if (iovec.iov_len <= 0) {
            log.debug("__syscall_readv:  iovec is empty or negetive: {}\n", .{iovec.iov_len});
            continue;
        }
        const iovecLength: usize = @intCast(iovec.iov_len);
        const writeSlice = (iovec.iov_base orelse {
            log.err("__syscall_readv:  iovec base is null\n", .{});
            return syscallUtil.ERROR_RETURN;
        })[0..iovecLength];
        const readBytes = files.readWithOffset(fd, bytesRead, writeSlice) catch |err| {
            log.err("__syscall_readv:  failed to read file: {}\n", .{err});
            return syscallUtil.ERROR_RETURN;
        };
        bytesRead += readBytes;
        if (readBytes < iovecLength) {
            log.debug("__syscall_readv:  reached end of file saj\n", .{});
            break;
        }
    }
    log.debug("__syscall_readv:  read 0x{X} bytes, from 0x{X} to 0x{X} in file, file size: 0x{X}\n", .{
        bytesRead,
        file.offset,
...
fn __syscall_writev(fd: u32, iov: ?[*]IoVec, iovcnt: i32) u32 {
    log.debug("++__syscall_writev()\n", .{});
    defer {
        log.debug("--__syscall_writev()\n", .{});
    }
    if (fd > files.fds.len) {
        log.err("__syscall_writev:  fd is out of range: {}\n", .{fd});
        return syscallUtil.ERROR_RETURN;
    }
    if (iovcnt <= 0) {
        log.err("__syscall_writev:  iovcnt is below or equal 0: {}\n", .{iovcnt});
        return syscallUtil.ERROR_RETURN;
    }
    const liovcnt: u32 = @intCast(iovcnt);
    const file = files.fds[fd] orelse {
    ...
    const iovSlice: []IoVec = (iov orelse {
        log.err("__syscall_writev:  iov is null\n", .{});
        return syscallUtil.ERROR_RETURN;
    })[0..liovcnt];
    var wroteBytes: u32 = 0;
    for (iovSlice) |iovec| {
        if (iovec.iov_len <= 0) {
            log.debug("__syscall_writev:  iovec is empty or negetive: {}\n", .{iovec.iov_len});
            continue;
        }
        const iovecLength: usize = @intCast(iovec.iov_len);
        const writeSlice = (iovec.iov_base orelse {
    ...
    log.debug("__syscall_writev:  wrote 0x{X} bytes, from 0x{X} to 0x{X} in file, file size: 0x{X}\n", .{
        wroteBytes,
        file.offset,
