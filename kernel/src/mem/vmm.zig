...
pub fn initVmm() void {
    ...
    kmalloc.init();
    log.info("vmm initilized\n", .{});
}

pub fn allocatePages(num: usize) ?[*]u8 {
    const page = virtualBitMap.alloc(num) catch |err| {
        log.err("vmm.allocatePages:  failed to allocate memory error: {}\n", .{err});
        return null;
    };
    for (0..num) |i| {
        ...
        if (paging.getPageTableEntryRecursivlyAlways(pageAddr)) |pageTableEntry| {
            if (pageTableEntry.flags.used == 0) {
                ...
                log.debug("vmm.allocatePages:  allocated page at: 0x{X} size: 0x{X}\n", .{ pageAddr, memory.physPageSizes });
            } else {
                log.err("vmm.allocatePages:  page is already used\n", .{});
                return null;
            }
        } else |err| {
            if (err == paging.PageErrors.IsBigPage) {
                ...
                const physPage = pmm.physBitMap.alloc(1) catch |physAllocErr| {
                    log.err("vmm.allocatePages:  failed to allocate physical page error: {}\n", .{physAllocErr});
                    return null;
                };
                ...
                _ = paging.setPageTableEntryRecursivlyAlways(pageAddr, physPageAddr, .{
                    ...
                    log.err("vmm.allocatePages:  failed to set page table entry: {}\n", .{setErr});
                    return null;
                };
                log.debug("vmm.allocatePages:  allocated page at: 0x{X} size: 0x{X}\n", .{ pageAddr, memory.physPageSizes });
            } else {
                log.err("vmm.allocatePages:  failed to get page table, error: {}\n\n", .{err});
                return null;
            }
        }
    }
...
pub fn freePages(address: [*]u8, num: usize) !void {
    ...
    for (0..num) |i| {
        const physAddr = paging.virtualToPhysical(@intFromPtr(address) + i * memory.PAGE_SIZE) catch |err| {
            log.err("vmm.freePage:  failed to get physical address: {}\n", .{err});
            return err;
        };
        ...
        paging.setPageTableEntryRecursivlyAlways(@intFromPtr(address) + i * memory.PAGE_SIZE, 0, .{}) catch |setErr| {
            log.err("vmm.allocatePage:  failed to set page table entry: {}\n", .{setErr});
            return setErr;
        };
    }
}
...
pub fn mapVirtualAddressRange(virtualAddr: u32, size: u32) ?[]u8 {
    if (!memory.isAligned(virtualAddr, memory.PAGE_SIZE)) {
        log.err("vmm.mapVirtualAddressRange:  virtual address is not aligned virtualAddr: 0x{X}\n", .{
            virtualAddr,
        });
        return null;
    }
    ...
    paging.idPagesRecursivly(virtualAddr, physicalAddr, lsize, true) catch |err| {
        log.err("vmm.mapVirtualAddressRange:  failed to id map virtual address range: {}\n", .{err});
        pmm.physBitMap.free(@ptrFromInt(physicalAddr), lsize / memory.PAGE_SIZE);
        return null;
    };
    // }
...
fn testVmmAlloc() void {
    log.info("vmm.testVmmAlloc:  start test\n", .{});
    const page = allocatePages(2) orelse {
        log.err("vmm.testVmmAlloc:  failed to allocate memory\n", .{});
        return;
    };
    const pageAddr = @intFromPtr(page);
    page[0] = 69;
    log.info("vmm.testVmmAlloc: success:  allocated address: 0x{X} first byte: {}\n", .{ pageAddr, page[0] });
    freePages(page, 2);
}
